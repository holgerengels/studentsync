...
class Untis extends Domain {
    ...
    async readTeachersWithMissingExternalIds() {
        ...
        try {
            ...
            console.error('Untis readTeachersWithMissingExternalIds failed', e);
            throw new Error('Untis readTeachersWithMissingExternalIds failed: ' + e.message);
        } finally {
            if (connection) await connection.end();
        }
    }
    ...
    async writeTeacherExternalId(name, foreignKey) {
        ...
        try {
            ...
            console.error(`Untis writeTeacherExternalId failed for ${name}`, e);
            throw new Error(`Untis writeTeacherExternalId failed for ${name}: ${e.message}`);
        } finally {
            if (connection) await connection.end();
        }
    }
