...
@customElement('page-asvschulkonsole')
export class PageASVSchulkonsole extends connect(store, LitElement) {
  @state()
  // @ts-ignore
  ...
  mapState(state: State) {
    return {
      _loading: state.asvschulkonsole.loading,
      _diffs: state.asvschulkonsole.entities,
      _error: state.asvschulkonsole.error,
    }
  }
  ...
  render() {
    ...
    return html`
      <mwc-top-app-bar id="bar" dense>
        ...
        <div slot="title">ASV → Schulkonsole</div>
        <mwc-icon-button icon="refresh" slot="actionItems" @click="${store.dispatch.asvschulkonsole.load}"></mwc-icon-button>
      </mwc-top-app-bar>
      <div class="board">
