...
export default createModel({
  ...
  effects: (store: Store) => ({
    async load() {
      ...
      if (Date.now() - state.asvschulkonsole.timestamp > 3000) {
        dispatch.asvschulkonsole.requestDiff();
        // @ts-ignore
        const resp = await fetch(endpoint.diff("asv", "schulkonsole"), endpoint.get);
        if (resp.ok) {
          const json = await resp.json();
          dispatch.asvschulkonsole.receivedDiff(json);
        }
        else {
          const message = await resp.text();
          // @ts-ignore
          dispatch.asvschulkonsole.error(message)
        }
      }
    },
    ...
    async sync() {
      ...
      dispatch.asvschulkonsole.requestSync();
      // @ts-ignore
      const resp = await fetch(endpoint.sync("asv", "schulkonsole"), endpoint.get);
      if (resp.ok) {
        await resp.json();
        dispatch.asvschulkonsole.receivedReport("lalilu");
      }
      else {
        const message = await resp.text();
        // @ts-ignore
